...
export const useSound = (initialSoundMode: 'on' | 'off' = 'on') => {
    ...
    const [currentIndex, setCurrentIndex] = useState<Record<SoundType, number>>({} as Record<SoundType, number>);

    const loadSound = useCallback((id: SoundType, customSoundData?: string | null) => {
    ...
    const reloadSounds = useCallback(() => {
        ...
        const initialIndexes: Record<SoundType, number> = {} as Record<SoundType, number>;
    ...
    const updateSound = useCallback((soundType: SoundType, soundData: string | null) => {
